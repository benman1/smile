...
public class RandomForest implements SoftClassifier<double[]>, Serializable {
    ...
    private final DecisionTree.SplitRule rule;
    ...
    static class TrainingTask implements Callable<Tree> {
         ...
         * Constructor.
         */
        TrainingTask(Attribute[] attributes, double[][] x, int[] y, int maxNodes, int nodeSize, int mtry, double subsample, DecisionTree.SplitRule rule, double[] classWeight, int[][] order, int[][] prediction) {
        ...
        public Tree call() {
            ...
            if (subsample == 1.0) {
                ...
                for (int l = 0; l < k; l++) {
                    ...
                    long size = Math.round(nj * classWeight[l]);
            ...
            int Nsamples = ((Long) Arrays.stream(samples).filter(s -> s == 0).count()).intValue();
            ...
            for (int i = 0; i < n; i++) {
                if (samples[i] == 0) {
                    ...
                    target[oob] = y[i];
                    if (p == y[i])
                        correct++; //correct += 1/(baselines[Arrays.binarySearch(labels, ((Integer)y[i]).intValue())]* labels.length);
     ...
     *                    positive).
     */
    public RandomForest(Attribute[] attributes, double[][] x, int[] y, int ntrees, int maxNodes, int nodeSize, int mtry, double subsample, DecisionTree.SplitRule rule, double[] classWeight) {
        ...
        if (classWeight == null) {
            double[] classProbs = DecisionTree.getClassProbs(y, labels);
            classWeight = new double[k];
            for (int i = 0; i < k; i++) {
                // classWeight[i] = 1;
                classWeight[i] = 1 / (labels.length * classProbs[i]);
            }
        }
    ...
    /**
     * prune ntree most useless trees
     */
    public void prune(int ntrees) {
        HashSet<Integer> modelSet = new HashSet<>();
        ...
        for (int i = 0; i < ntrees; i++) {
            double min = Math.min(weights);
            modelSet.remove(Arrays.asList(weights).indexOf(min));
        }
    ...
    public void sampleTrees(int ntrees) {
        boolean oversample = ntrees >= trees.size();
        List<Tree> model = new ArrayList<>(ntrees);
        HashSet<Integer> modelSet = new HashSet<>();
        Random rn = new Random();
        while (modelSet.size() < ntrees) {
            int m = rn.nextInt(trees.size());
            if (oversample || !modelSet.contains(m)) {
                model.add(this.trees.get(m));
                modelSet.add(m);
            }
        }
        this.trees = model;
    ...
    public Tree[] getForest() {
        Tree[] forest = new Tree[trees.size()];
